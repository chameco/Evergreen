...
base::entity::entity(vector<int> *_coords, map<string, string> *_data, chameleon::event::manager *_manager, int _curLevel) : base::physicalObject(_coords) {
	attrs = new map<string, int>();
	data = _data;
	if (data == NULL) {
		data = new map<string, string>();
		(*data)["facing"] = "up";
		(*data)["name"] = getType();//We need to implement amountCreated as a static;
	}
...
void base::entity::attack(base::group<base::physicalObject> *allSprites) {
	base::rect *collider;
	if ((*data)["facing"] == "up") {
...
void base::entity::update(base::group<base::physicalObject> *allSprites) {
	int temprequestx = requestx;
	int temprequesty = requesty;
	base::group<base::physicalObject> *notMe = new base::group<base::physicalObject>(allSprites);
	notMe->remove(this);
...
void base::entity::serialize(Archive &ar, const unsigned int version) {
	ar & boost::serialization::base_object<base::physicalObject>(*this);
}
